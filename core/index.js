...
    this.level = this.levels.debug; // default: info
...
    let tagStyle; // Menggunakan style yang lebih standar
    let emoji = ' ';

    // 🚀 PERBAIKAN STABILITAS: Menggunakan kombinasi warna standar (bgX.white/black)
    switch (tag) {
...
    }
    ...
    // Override gaya berdasarkan LEVEL (untuk ERROR/WARN)
...
    this.logger = logger; 
    this.modulePaths = new Map(); // name => path (optional to track)
  }
...
    // Overwrite existing plugin with same name (simple hot-replace)
...
      },
      // 🚀 PERBAIKAN: MENGGUNAKAN POLA DELETION YANG BENAR
      deleteMessage: async (key) => {
          try {
              // Asumsi: Framework Anda menggunakan pola Baileys (atau sejenisnya) untuk menghapus pesan.
              // Memerlukan akses ke this.mockWA.sock
...
              throw e; // Melempar error agar antilink dapat menangkap dan mencatat log kegagalan izin Admin.
          }
      },
      
      // ===== ✅ TAMBAHAN BARU: saveUsers() =====
      saveUsers: async () => {
          await this.saveData(true); // Memaksa penyimpanan data pengguna
      },
      // ===================================
...
            if (plugin?.name) this.registry.register(plugin, path.join(pluginsDir, f));
...
      const name = path.basename(file);
      try {
        // Import with cache-busting query to force reload in ESM environment
        const fileURL = pathToFileURL(file).href;
        const module = await import(fileURL + `?update=${Date.now()}`);
        const plugin = module.default || module;
        if (plugin?.name) {
          // replace existing plugin entry
          this.registry.register(plugin, file);
...
      // try to unregister by name if present
...
  async start() {
    console.clear();
    this.logger.info('CORE', 'Starting Engine v4.9 (Fixed Event Dispatcher)...');
